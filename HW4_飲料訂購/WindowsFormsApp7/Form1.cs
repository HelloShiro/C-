...
namespace WindowsFormsApp7
{
    public partial class Form1 : Form
    {
        ...
        int disAmount1 = 0, disAmount2 = 0, disAmount3 = 0, disAmount4 = 0, disAmount5 = 0;
        ...
        {
            DialogResult dialogResult;
            dialogResult = MessageBox.Show("第二件六折", "確認第二件六折?", MessageBoxButtons.OKCancel);
            if(dialogResult == DialogResult.OK)
            {
                disAmounts = (amount01 + amount02 + amount03 + amount04 + amount05) / 2;

                if (disAmounts >= amount01)
                {
                    disAmount1 = amount01;
                    disAmounts = disAmounts - disAmount1;
                    if (disAmounts >= amount02)
                    {
                        disAmount2 = amount02;
                        disAmounts = disAmounts - disAmount2;
                        if (disAmounts >= amount03)
                        {
                            disAmount3 = amount03;
                            disAmounts = disAmounts - disAmount3;
                            if (disAmounts >= amount04)
                            {
                                disAmount4 = amount04;
                                disAmounts = disAmounts - disAmount4;
                                if (disAmounts >= amount05)
                                {
                                    disAmount5 = amount05;
                                    disAmounts = disAmounts - disAmount5;
                                }
                                else
                                {
                                    disAmount5 = amount05 / 2;
                                    disAmounts = disAmounts - disAmount5;
                                }
                            }
                            else
                            {
                                disAmount4 = amount04 / 2;
                                disAmounts = disAmounts - disAmount4;
                            }
                        }
                        else
                        {
                            disAmount3 = amount03 / 2;
                            disAmounts = disAmounts - disAmount3;
                        }
                    }
                    else
                    {
                        disAmount2 = amount02 / 2;
                        disAmounts = disAmounts - disAmount2;
                    }
                }
                else
                {
                    disAmount1 = amount01 / 2;
                    disAmounts = disAmounts - disAmount1;
                }

                totalAfterDiscounts =
                    disAmount1 * price01 * 0.6 +
                    disAmount2 * price02 * 0.6 +
                    disAmount3 * price03 * 0.6 +
                    disAmount4 * price04 * 0.6 +
                    disAmount5 * price05 * 0.6 +
                    (amount01 - disAmount1) * price01 +
                    (amount02 - disAmount2) * price02 +
                    (amount03 - disAmount3) * price03 +
                    (amount04 - disAmount4) * price04 +
                    (amount05 - disAmount5) * price05;
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                strOrderList += strBuy1free1Clicked + "\n";
                strOrderList += strSixtyOff + "\n\n";
